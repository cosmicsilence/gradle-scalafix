...
By default, Scalafix won't run any rules unless they are specified via configuration file or command line argument.

>**TIP:** In order to decide what rules to pick and how to configure them, please check out Scalafix's [built-in 
...
is also possible to inform an arbitrary path via the plugin extension in the build script. See the [extension](#extension)
...
file. For such one-off commands, you can inform the rule(s) you want to run using the `scalafix.rules` property:
...
|`includes`                |`SetProperty<String>` |\<empty\>      |Ant-like pattern to filter what Scala source files should be processed by Scalafix. By default all files are included. |
|`excludes`                |`SetProperty<String>` |\<empty\>      |Ant-like pattern to exclude Scala source files from being processed by Scalafix. By default no files are excluded. |
|`autoConfigureSemanticdb` |`Boolean`             |`true`         |Used to indicate whether the Scalafix plugin should auto-configure the SemanticDB compiler plugin. This is mandatory to be able to run Scalafix's semantic rules. If set to `true`, the Scalafix Gradle tasks will cause the Scala compiler to kick in whenever they are run. If your project only uses syntactic rules, then it's highly recommended that this property is set to `false` as the SemanticDB compiler plugin may cause a substantial slow down in your build. The Scalafix plugin auto-configures only the necessary parameters of the SemanticDB compiler plugin. If you need to use more advanced settings, please consult [this section](https://scalacenter.github.io/scalafix/docs/users/installation.html#exclude-files-from-semanticdb) on the Scalafix website as well as the [SemanticDB docs](https://scalameta.org/docs/semanticdb/guide.html#scalac-compiler-plugin). Any additional SemanticDB settings can be informed through the `ScalaCompile` task as shown earlier. |
...
dependencies in your build script. The only requirement is to have your custom rules published as a jar file to a Maven
repository of your choice and informed in the dependency section using the `scalafix` configuration. Example:
